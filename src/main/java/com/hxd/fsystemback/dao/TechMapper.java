...
public interface TechMapper {
    ...
    @Select("SELECT t.*, p.name AS productname, p.standard AS productstandard,p.prewarn,p.note AS productnote, pt.name AS partsname, pt.standard AS partsstandard,pt.`group` As partsGroup FROM tech AS t LEFT JOIN product p ON t.productid = p.id LEFT JOIN parts pt ON t.partsid = pt.id ORDER BY p.name ASC")
